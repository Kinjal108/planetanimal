          ...
          <div class="row">
            <div class="col-2"> 
              @if(getcong('site_logo'))                 
                <a href="{{ URL::to('/') }}" title="logo" class="logo"><img src="{{ URL::asset('public/'.getcong('site_logo')) }}" alt="logo" title="logo"></a>
              @else
                <a href="{{ URL::to('/') }}" title="logo" class="logo"><img src="{{ URL::asset('public/site_assets/images/logo.png') }}" alt="logo" title="logo"></a>                          
              @endif
                ...
                <div id="menu" class="collapse header-menu">
                  <ul class="nav vfx-item-nav">
                    ...
                    <li><a href="{{ URL::to('livetv') }}" class="{{classActivePathSite('livetv')}}" title="{{trans('words.live_tv')}}">{{trans('words.live_tv')}}</a> <span class="arrow"></span>
                      <ul class="dm-align-2 mega-list">
                        @foreach(\App\Models\TvCategory::where('status','1')->orderBy('category_name')->get() as $tv_cat)
      ...
      <div class="col-3"> 
        <div class="right-sub-item-area">
          ...
          <div class="subscribe-btn-item">
            <a href="{{ URL::to('membership_plan') }}" title="subscribe"><img src="{{ URL::asset('public/site_assets/images/ic-subscribe.png') }}" alt="ic-subscribe" title="ic-subscribe"></a>
          </div>
          @if(Auth::check())
            ...
            <div class="user-name">
              <span>
                @if(Auth::User()->user_image AND file_exists(public_path('upload/'.Auth::User()->user_image)))
                  <img src="{{ URL::asset('public/upload/'.Auth::User()->user_image) }}" alt="profile_img" title="{{Auth::User()->name,6}}" id="userPic">
                @else  
                    <img src="{{ URL::asset('public/site_assets/images/user-avatar.png') }}" alt="profile_img" title="{{Auth::User()->name,6}}" id="userPic">
                @endif
          ...
          <div class="signup-btn-item">
            <a href="{{ URL::to('login') }}" title="login"><img src="{{ URL::asset('public/site_assets/images/ic-signup-user.png') }}" alt="ic-signup-user" title="signup-user"><span>{{trans('words.login_text')}}</span></a>
          @endif  
          </div>
        </div>        
      </div>
