...
@section('head_image', URL::to('public/'.$series_info->series_poster))
  ...
  <div class="container-fluid">
    ...
    <div class="play-icon-item">
      ...
      <div class="video-post-date">
        ...
        <div class="video-watch-share-item">
          ...
          @if($series_info->imdb_rating)           
           <span class="video-imdb-view"><img src="{{URL::to('public/site_assets/images/imdb-logo.png')}}" alt="imdb-logo" title="imdb-logo" />{{$series_info->imdb_rating}}</span> 
          @endif 
           &nbsp;
      ...
      <div class="dtl-poster-img">
        <img src="{{URL::to('public/'.$series_info->series_poster)}}" alt="tv-series-poster-1" title="{{stripslashes($series_info->series_name)}}" />
      </div>
    </div>
    </div>
    <div class="col-lg-5 col-md-12 col-sm-12 col-xs-12 mb-4"> 
      ...
      <ul class="dtl-list-link dtl-link-col">
        <li><a href="#" title="Seasons">{{\App\Models\Series::getSeriesTotalSeason($series_info->id)}} Seasons</a></li>
        <li><a href="#" title="Episodes">{{\App\Models\Series::getSeriesTotalEpisodes($series_info->id)}} Episodes</a></li>       
      </ul>
      ...
      @if(!is_null($series_info->actor_id)>0)
        ...
        <span class="des-bold-text"><strong>{{trans('words.actors')}}:</strong> 
          ...
          <a href="{{ URL::to('actors/'.App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_slug')) }}/{{$actor_ids}}" title="actors">{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}</a><?php if (($i+1) != $n) echo $a = ',';?>
      ...
      <span class="des-bold-text"><strong>{{trans('words.directors')}}:</strong> 
              ...
              <a href="{{ URL::to('directors/'.App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_slug')) }}/{{$director_ids}}" title="directors">{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}</a><?php if (($i+1) != $n) echo $a = ',';?>
      ...
      <div class="row">
        ...
        <div class="season-item-related owl-carousel">
          @foreach($season_list as $season_data)
              ...
              @php
                    if(getImageSizeFromURL(URL::to('/'.str_replace(" ", "%20", $season_data->season_poster)))!=false)
                    {
                    
                      $season_image_size = getImageSizeFromURL(URL::to('public/'.str_replace(" ", "%20", $season_data->season_poster)));
