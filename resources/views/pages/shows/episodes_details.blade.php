...
<link rel="stylesheet" type="text/css" href="{{ URL::asset('public/site_assets/player/content/global.css') }}">
<script type="text/javascript" src="{{ URL::asset('public/site_assets/player/java/FWDEVPlayer.js') }}"></script>
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
        ...
        <div class="row mt-30">
          ...
          <ul class="dtl-list-link dtl-link-col">
            ...
            <li><a href="#" title="{{App\Models\Season::getSeasonInfo($episode_info->episode_season_id,'season_name')}}">{{App\Models\Season::getSeasonInfo($episode_info->episode_season_id,'season_name')}}</a></li>       
          </ul>
        <div class="video-post-date">
            ...
            @if($episode_info->imdb_rating)
             <span class="video-imdb-view"><img src="{{URL::to('public/site_assets/images/imdb-logo.png')}}" alt="imdb-logo" title="imdb-logo"/>{{$episode_info->imdb_rating}}</span>
             @endif
        ...
        <ul class="actor-video-link">
          ...
          <li><a href="{{ URL::to('shows?genre_id='.$genres_ids) }}" title="{{App\Models\Genres::getGenresInfo($genres_ids,'genre_name')}}">{{App\Models\Genres::getGenresInfo($genres_ids,'genre_name')}}</a></li>
          @endforeach
          <li><a href="{{ URL::to('shows?lang_id='.$series_info->series_lang_id) }}" title="{{\App\Models\Language::getLanguageInfo($series_info->series_lang_id,'language_name')}}">{{\App\Models\Language::getLanguageInfo($series_info->series_lang_id,'language_name')}}</a></li>  
        ...
        <ul>
          ...
          @if($series_info->actor_id) 
          <li class="tab2">
          <label for="tab2">{{trans('words.actors')}}</label>
          </li>
          @endif
          @if($series_info->director_id) 
          <li class="tab3">
        ...
        <div class="tab2">
          ...
          <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12 col-6">
            <div class="actors-member-item"> 
              <a href="{{ URL::to('actors/'.App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_slug')) }}/{{$actor_ids}}" title="actors details"> 
                @if(App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_image')) 
                <img src="{{URL::to('public/'.App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_image'))}}" alt="{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}" title="{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}">  
                @else
                  <img src="{{URL::to('public/images/user_icon.png')}}" alt="{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}" title="{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}"> 
                @endif
                
                
                <span>{{App\Models\ActorDirector::getActorDirectorInfo($actor_ids,'ad_name')}}</span> 
              </a>
            </div>
          </div>
          @endforeach
        ...
        <div class="tab3">
          ...
          <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12 col-6">
            <div class="actors-member-item"> 
              <a href="{{ URL::to('directors/'.App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_slug')) }}/{{$director_ids}}" title="directors details"> 
                @if(App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_image')) 
                <img src="{{URL::to('public/'.App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_image'))}}" alt="{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}" title="{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}"> 
                @else
                  <img src="{{URL::to('public/images/user_icon.png')}}" alt="{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}" title="{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}"> 
                @endif

                <span>{{App\Models\ActorDirector::getActorDirectorInfo($director_ids,'ad_name')}}</span> 
              </a>
            </div>
          </div>
          @endforeach
             ...
             <div class="video-img"> 
              ...
              @if($episode_data->video_access=="Paid")       
                <div class="vid-lab-premium">
                  <img src="{{ URL::asset('public/site_assets/images/ic-premium.png') }}" alt="premium" title="premium">
                </div> 
                @endif

              <img src="{{URL::to('public/'.$episode_data->video_image)}}" alt="{{stripslashes($episode_data->video_title)}}" title="{{stripslashes($episode_data->video_title)}}">         
             </div>
      ...
      <div class="row">
        ...
        <div class="season-item-related owl-carousel">
          @foreach($season_list as $season_data)
            {{-- k108 start --}}
            @php
                $season_image_size = getImageSizeFromURL(
                    URL::to('public/'.str_replace(" ", "%20", $season_data->season_poster))
                );

                if ($season_image_size && isset($season_image_size['width'], $season_image_size['height'])) {
                    $orientation = $season_image_size['width'] > $season_image_size['height'] 
                        ? "landscape" 
                        : "portrait";
                } else {
          ...
          <a href="{{ URL::to('shows/'.$series_info->series_slug.'/seasons/'.$season_data->season_slug.'/'.$season_data->id) }}" title="{{stripslashes($season_data->season_name)}}">
             <div class="video-img">  
              ...
              <img src="{{URL::to('public/'.$season_data->season_poster)}}" alt="{{$season_data->season_name}}" alt="{{stripslashes($season_data->season_name)}}" title="{{stripslashes($season_data->season_name)}}">         
             </div>
             <div class="season-title-item">
