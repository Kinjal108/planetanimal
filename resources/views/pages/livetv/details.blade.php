...
@section('head_image', URL::to('public/'.$tv_info->channel_thumb) )
...
<div class="page-content-area vfx-item-ptb pt-3">
            ...
            <div class="modal-content bg-dark-2 text-light">
              ...
              <div class="social-media-modal">
                <ul>
                  ...
                  <a href="javascript:void(0);" onclick="copyLink('{{ share_url_get('livetv',$tv_info->channel_slug,$tv_info->id) }}')" class="instagram-icon">
  <i class="ion-social-instagram"></i>
      ...
      <div class="dtl-poster-img">
        <img src="{{URL::to('public/'.$tv_info->channel_thumb)}}" alt="{{stripslashes($tv_info->channel_name)}}" title="{{stripslashes($tv_info->channel_name)}}" />
      </div>
    </div>
    </div>
    <div class="col-lg-5 col-md-12 col-sm-12 col-xs-12 mb-4"> 
      ...
      <ul class="dtl-list-link">
        ...
        <li><a href="{{ URL::to('livetv/?cat_id='.$tv_info->channel_cat_id) }}" title="{{App\Models\TvCategory::getTvCategoryInfo($tv_info->channel_cat_id,'category_name')}}">{{App\Models\TvCategory::getTvCategoryInfo($tv_info->channel_cat_id,'category_name')}}</a></li>
        ...
        <div class="tv-season-video-carousel owl-carousel">
          ...
          <a href="{{ URL::to('livetv/details/'.$related_data->channel_slug.'/'.$related_data->id) }}" title="{{stripslashes($related_data->channel_name)}}">
             <div class="video-img">
              ...
              <img src="{{URL::to('public/'.$related_data->channel_thumb)}}" alt="{{stripslashes($related_data->channel_name)}}" title="{{stripslashes($related_data->channel_name)}}">         
             </div>       
          </a>
          </div>
          @endforeach    
