...
@section('head_image', URL::to('public/'.$sports_info->video_image) )
  ...
  <div class="container-fluid">
    ...
    <div class="detail-poster-area">
     ...
     <div class="play-icon-item">
				<a class="icon" href="{{ URL::to('earthling/watch/'.$sports_info->video_slug.'/'.$sports_info->id) }}" title="play">
            ...
            <div class="modal-content bg-dark-2 text-light">
              ...
              <div class="social-media-modal">
                <ul>
                  ...
                  {{-- k108 start --}}
                  <li>
                    <a href="javascript:void(0);" class="instagram-icon" onclick="copyAndOpenInstagram('{{ share_url_get('sports',$sports_info->video_slug,$sports_info->id) }}')">
      ...
      <div class="dtl-poster-img">
        <img src="{{URL::to('public/'.$sports_info->video_image)}}" alt="{{stripslashes($sports_info->video_title)}}" title="{{stripslashes($sports_info->video_title)}}" />
      </div>
    </div>
    </div>
    <div class="col-lg-5 col-md-12 col-sm-12 col-xs-12 mb-4"> 
      <div class="poster-dtl-item">
      <h2><a href="{{ URL::to('public/earthling/watch/'.$sports_info->video_slug.'/'.$sports_info->id) }}" title="{{stripslashes($sports_info->video_title)}}">{{stripslashes($sports_info->video_title)}}</a></h2>
      <ul class="dtl-list-link">
         
        <li><a href="{{ URL::to('public/earthling/?cat_id='.$sports_info->sports_cat_id) }}" title="{{App\Models\Earthling_category::getSportsCategoryInfo($sports_info->sports_cat_id,'category_name')}}">{{App\Models\Earthling_category::getSportsCategoryInfo($sports_info->sports_cat_id,'category_name')}}</a></li>
    ...
    <div class="video-carousel-area vfx-item-ptb related-video-item">
      ...
      <div class="row">
        ...
        <div class="tv-season-video-carousel owl-carousel">
          ...
          <a href="{{ URL::to('earthling/details/'.$sports_data->video_slug.'/'.$sports_data->id) }}" title="{{stripslashes($sports_data->video_title)}}">
             <div class="video-img">
              ...
              <img src="{{URL::to('public/'.$sports_data->video_image)}}" alt="{{stripslashes($sports_data->video_title)}}" title="{{stripslashes($sports_data->video_title)}}">         
             </div>       
          </a>
          </div>
          @endforeach    
...
function copyAndOpenInstagram(url) {
    ...
    navigator.clipboard.writeText(url).then(() => {
        // alert('Link copied! Now Instagram will open, you can paste it there.');
       // Open Instagram (mobile app link)
        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
        if(isMobile){
            window.location.href = 'instagram://app'; // Opens Instagram app
        } else {
