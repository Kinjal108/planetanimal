...
@section('head_image', URL::to('public/'.$movies_info->video_image))
				...
				<div class="video-posts-data mt-0 mb-0">
					...
					<div class="video-post-date">
						...
						@if($movies_info->imdb_rating)           
						 <span class="video-imdb-view"><img src="{{URL::to('public/site_assets/images/imdb-logo.png')}}" alt="imdb-logo" title="imdb-logo" />{{$movies_info->imdb_rating}}</span> 
						@endif 
					</div>
				<ul class="actor-video-link">
				  @foreach(explode(',',$movies_info->movie_genre_id) as $genres_ids)
				  <li><a href="{{ URL::to('movies?genre_id='.$genres_ids) }}" title="{{App\Models\Genres::getGenresInfo($genres_ids,'genre_name')}}">{{App\Models\Genres::getGenresInfo($genres_ids,'genre_name')}}</a></li>
				  @endforeach
				  <li><a href="{{ URL::to('movies?lang_id='.$movies_info->movie_lang_id) }}" title="{{App\Models\Language::getLanguageInfo($movies_info->movie_lang_id,'language_name')}}">{{App\Models\Language::getLanguageInfo($movies_info->movie_lang_id,'language_name')}}</a></li>           
				</ul>
          ...
          <a href="{{ URL::to('movies/details/'.$movies_data->video_slug.'/'.$movies_data->id) }}" title="{{stripslashes($movies_data->video_title)}}">
             <div class="video-img">          
              ...
              <div class="vid-lab-premium">
                <img src="{{ URL::asset('public/site_assets/images/ic-premium.png') }}" alt="ic-premium" title="ic-premium">
              </div> 
              @endif  

              <span class="video-item-content">{{stripslashes($movies_data->video_title)}}</span> 
              <img src="{{URL::to('public/'.$movies_data->video_image_thumb)}}" alt="{{stripslashes($movies_data->video_title)}}" title="{{stripslashes($movies_data->video_title)}}">         
             </div>       
          </a>
          </div>
          @endforeach
