...
<!-- Start Breadcrumb -->
<div class="breadcrumb-section bg-xs" style="background-image: url('{{ URL::asset('public/site_assets/images/breadcrum-bg.jpg') }}')">
...
<div class="video-shows-section sport-video-block vfx-item-ptb">
  ...
  <div class="row">
    <div class="col-md-12">
    <div class="vfx-item-section">
      <h3>{{trans('words.earthling_text')}}</h3>      
    </div>
    <div class="tv-season-video-carousel owl-carousel">
      ...
      <div class="single-video">
        ...
        <a href="{{ URL::to('earthling/details/'.App\Models\Sports::getSportsInfo($sport_data->post_id,'video_slug').'/'.$sport_data->post_id) }}" title="{{App\Models\Sports::getSportsInfo($sport_data->post_id,'video_title')}}">
           <div class="video-img">
            <span class="video-item-content">{{App\Models\Sports::getSportsInfo($sport_data->post_id,'video_title')}}</span>
            <img src="{{URL::to('public/'.App\Models\Sports::getSportsInfo($sport_data->post_id,'video_image'))}}" alt="{{App\Models\Sports::getSportsInfo($sport_data->post_id,'video_title')}}" title="{{App\Models\Sports::getSportsInfo($sport_data->post_id,'video_title')}}" />         
           </div>                          
        </a>
      </div>
      @endforeach
    ...
    <div class="tv-season-video-carousel owl-carousel">
      ...
      <div class="single-video">
        <div class="watchlist-item"><a href="{{URL::to('watchlist/remove')}}?post_id={{$livetv_data->post_id}}&post_type=LiveTV" title="Remove"><i class="fa fa-times"></i>{{trans('words.remove')}}</a></div>
          ...
          <a href="{{ URL::to('livetv/details/'.App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_slug').'/'.$livetv_data->post_id) }}" title="{{App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_name')}}">
           <div class="video-img">       
               
            <span class="video-item-content">{{App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_name')}}</span>
            <img src="{{URL::to('/'.App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_thumb'))}}" alt="{{App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_name')}}" title="{{App\Models\LiveTV::getLiveTvInfo($livetv_data->post_id,'channel_name')}}" />         
           </div>                          
        </a>
